  ...
  if _ctx is not None and _ctx._thread_local_data.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        ...
        "SparseFeatureCross", name, _ctx.post_execution_callbacks, indices,
...
def SparseFeatureCross(indices, values, shapes, dense, hashed_output, num_buckets, out_type, internal_type, name=None):
  return sparse_feature_cross(indices=indices, values=values, shapes=shapes, dense=dense, hashed_output=hashed_output, num_buckets=num_buckets, out_type=out_type, internal_type=internal_type, name=name)
SparseFeatureCross.__doc__ = sparse_feature_cross.__doc__
SparseFeatureCross = _doc_controls.do_not_generate_docs(_kwarg_only(SparseFeatureCross))
...
def sparse_feature_cross_v2(indices, values, shapes, dense, hashed_output, num_buckets, hash_key, out_type, internal_type, name=None):
  ...
  if _ctx is not None and _ctx._thread_local_data.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        _ctx._context_handle, _ctx._thread_local_data.device_name,
        "SparseFeatureCrossV2", name, _ctx.post_execution_callbacks, indices,
        values, shapes, dense, "hashed_output", hashed_output, "num_buckets",
...
def SparseFeatureCrossV2(indices, values, shapes, dense, hashed_output, num_buckets, hash_key, out_type, internal_type, name=None):
  return sparse_feature_cross_v2(indices=indices, values=values, shapes=shapes, dense=dense, hashed_output=hashed_output, num_buckets=num_buckets, hash_key=hash_key, out_type=out_type, internal_type=internal_type, name=name)
SparseFeatureCrossV2.__doc__ = sparse_feature_cross_v2.__doc__
SparseFeatureCrossV2 = _doc_controls.do_not_generate_docs(_kwarg_only(SparseFeatureCrossV2))
tf_export("raw_ops.SparseFeatureCrossV2")(SparseFeatureCrossV2)
