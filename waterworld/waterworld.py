...
class WaterWorld(PyGameWrapper):
    ...
    def getGameState(self):
        ...
        state = list()
        state.append(self.player.vel.x)
        state.append(self.player.vel.y)

        for c in self.creeps:
            state.append(self.player.pos.x - c.pos.x)
            state.append(self.player.pos.y - c.pos.y)
            state.append(c.direction.x)
            state.append(c.direction.y)
            if c.TYPE == "GOOD":
                state.append(1)
            else:
                state.append(-1)
